workdir <- "C:/Users/mwedel/Dropbox/Courses/BMSO758E(2020)/Projects/Project5"
setwd(workdir)
...
plot(out2.1)
plot(out2.2)
#Regression of posterior probabilities on Urbanisation factor
urbaniz<-as.factor(tapply(urb,Respondent,mean))
levels(urbaniz)[1]="Rural"
levels(urbaniz)[2]="Urban"
out=lm(log(post2/(1-post2))~urbanis)
summary(out)
...
out=lm(log(post2/(1-post2))~urbaniz)
summary(out)
summary(urb)
summary(urbaniz)
